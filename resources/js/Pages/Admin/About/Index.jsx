...
export default function AboutIndex({ auth, abouts }) {
  ...
  const handleSubmit = (e) => {
    ...
    if (editingId) {
      router.post(
        route('admin.about.update', editingId),
        { ...data, _method: 'PUT' },
        {
          onSuccess: () => {
            reset();
      ...
      post(route('admin.about.store'), { onSuccess: () => reset() });
    }
  };
          ...
          <div className="bg-black text-white rounded-xl shadow-sm p-6 md:p-8 space-y-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <input
                ...
                onChange={(e) =>
                  setData('medsos', { ...data.medsos, instagram: e.target.value })
                }
                className="w-full p-2 bg-black border-b-2 border-emerald-500 focus:border-emerald-400 focus:outline-none"
              />
              <input
                ...
                onChange={(e) =>
                  setData('medsos', { ...data.medsos, linkedin: e.target.value })
                }
                className="w-full p-2 bg-black border-b-2 border-emerald-500 focus:border-emerald-400 focus:outline-none"
              />
              <input
                ...
                onChange={(e) =>
                  setData('medsos', { ...data.medsos, github: e.target.value })
                }
                className="w-full p-2 bg-black border-b-2 border-emerald-500 focus:border-emerald-400 focus:outline-none"
              />
            ...
            <div className="grid gap-4 sm:grid-cols-2 md:grid-cols-3">
              {abouts.map((m) => (
                <div
                  key={m.id}
                  className="bg-emerald-900 rounded-lg p-4 flex flex-col items-center"
                >
                  <img
                    src={`/storage/${m.img_url}`}
                    alt={m.name}
                  ...
                  <div className="flex gap-2 mt-2 text-sm">
                    {m.medsos?.instagram && (
                      <a href={m.medsos.instagram}>IG</a>
                    )}
                    {m.medsos?.linkedin && (
                      <a href={m.medsos.linkedin}>LinkedIn</a>
                    )}
